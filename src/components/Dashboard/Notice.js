...
const Notice = () => {
	...
	return <div>{addNotice ? <AddNotice /> : <ViewNotice />}</div>;
};
		...
		return [
			...
			},
			{
				...
				Cell: ({ row }) => {
					...
					return <div>{moment(created_at).format("DD MMMM, YYYY")}</div>;
				},
			},
			{
				Header: "Action",
		...
		<div className="text-primary p-3 m-3 bg-white rounded-md ">
			...
			<form className="p-3 " onSubmit={handleAddNotice}>
				...
				<div className="mb-3 flex flex-col items-start w-full">
					...
					<input
						className="form-control  block w-full px-3  rounded py-2 text-base  font-normal bg-clip-padding bg-bgclr outline-none focus:outline-none active:outline-none"
						type="file"
						id="img"
